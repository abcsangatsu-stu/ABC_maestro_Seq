...
**最終更新:** 2026-03-24
**ステータス:** v9.0（ピアノロールモード / RT周波数プレビュー音量連動）
**最新ファイル:** `ABC_maestro_Seq_v9_0.html`（単一HTML）
...
│ └─────────────────────────────────────────────────────────┘   │
│  ▼playhead                                                    │
│  Selected: A4 L1 bar 1.1 dur:12 wave:△  [▲][▼][✕]            │
└──────────────────────────────────────────────────────────────┘
```

**描画仕様:**
- Canvasベース（`<canvas id="prCanvas">`）、DPR対応
- 3レーン: 各28px高、レーン間1pxギャップ、上下にバー番号行16px
...
| ノート本体ドラッグ | 時間軸上で移動 |
| ノート右端ドラッグ | 長さ変更 |
| ▲▼ボタン / 矢印キー | 選択ノートの音程変更（プレビュー付き） |
| 右クリック / Ctrl+クリック | ノート削除 |
| Deleteキー / ✕ボタン | 選択中ノート削除 |
...
**モード切替:**
- SONG欄ヘッダーの `[Step Grid / Piano Roll]` ボタンでトグル
- Piano Roll時: RT Song行 / RT CH Block / RT Pattern Bar / RT Grid を非表示
...
// マウス操作
prScreenToGrid(e)                    // マウス座標 → {tick, midi, mx, my}
prSnapTick(tick)                     // tick → snap吸着位置
prNoteAtPos(tick, midi)              // 座標上のノートindex（-1=なし）
prIsResizeHandle(noteIdx, mx)        // リサイズハンドル判定
...
v9.x           📋 ピアノロール改善（音域拡張 / velocity編集 / コピペ / ループポイント連動 / MIDI出力）
