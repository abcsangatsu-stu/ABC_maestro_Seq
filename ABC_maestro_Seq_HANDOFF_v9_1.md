...
**最終更新:** 2026-03-24
**ステータス:** v9.1（ピアノロール複数選択・コピペ / Zoom改善 / Snap拡張）
**最新ファイル:** `ABC_maestro_Seq_v9_1.html`（単一HTML）
...
prCurrentLane = 0           // 現在のアクティブレーン（0-2）
...
SONG欄直下に配置。`pianoRollMode = true` 時のみ表示。シングルレーン×3方式。
...
- 3レーン: 各44px高（v9.1拡大、旧28px）、レーン間1pxギャップ、上下にバー番号行18px（旧16px）
- Canvas全体高: 182px（旧130px）
...
prZoomStep(dir)                      // v9.1: ボタン/ホイール用ズームステップ（dir: +1/-1）
clearPianoRollNotes()                // 全ノート削除（confirm付き）
renderPianoRoll()                    // Canvas再描画

// 複数選択（v9.1〜）
prIsSelected(idx)                    // ノートが選択中か判定
prSelectOnly(idx)                    // 単一選択（他をクリア）
prToggleSelect(idx)                  // 選択トグル（Shift+click用）
prDeselectAll()                      // 全選択解除
prFixSelectionAfterSplice(removedIdx) // splice後のインデックス修正

// コピー＆ペースト（v9.1〜）
prCopySelected()                     // 選択ノートをprNoteClipboardへコピー
prPasteNotes()                       // playhead位置にペースト

// マウス操作
prScreenToTick(e)                    // マウス座標 → {tick, mx, my, lane}
prSnapTick(tick)                     // tick → snap吸着位置
prNoteAtPos(tick, lane)              // 座標上のノートindex（-1=なし）
prIsResizeZone(noteIdx, mx)          // リサイズハンドル判定

// 再生
prPlayTick(absoluteStep)             // 絶対ステップ位置のノートを発音
previewFreqTone(freq)                // サイン波プレビュー（RT Vol連動）

// オフライン書き出し
renderPianoRollOffline(left, right, totalFrames, sampleRate)

// Import / 変換
importRTToPianoRoll()                // rtSong全体 → pianoRollNotes変換（replace/add選択）
...
v9.x           📋 ピアノロール改善（音域拡張 / velocity編集 / ループポイント連動 / MIDI出力）
