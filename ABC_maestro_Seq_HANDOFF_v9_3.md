...
**最終更新:** 2026-04-12
**ステータス:** v9.3（UI改善 / パターン命名修正 / リネーム連動）
**最新ファイル:** `ABC_maestro_Seq_v9_3.html`（単一HTML）
...
- **ループボタン配置変更**
  - BTN ROW上段でLoop（↻）をPattern Play（▶青）の左側に移動
  - 従来: `▶PAT ▶SONG ■STOP ↻LOOP` → 新: `↻LOOP ▶PAT ▶SONG ■STOP`
- **Song欄「+」ボタンのパターン命名修正**
  - Back: ブランク名(`''`)で作成していたのを `nextPatternName()` で自動命名に変更
  - RT: 同様に `nextRTPatternName()` で自動命名に変更
  - 初期状態（パターンAのみ）から「+」で追加すると B, C, D... と順に命名される
- **パターンリネーム連動（baseName伝播）**
  - Song cellダブルクリックでリネーム時、同じ `baseName` を持つ派生パターンも連動して変更
  - 例: A→Cにリネーム → A1→C1, A2→C2 も自動更新
  - Back / RT 両方対応（`isR` パラメータで切り替え）
  - 既存の `renamePatternBase()` 関数のロジックをインライン化して適用
...
### v9.3（2026-04-12）
- **ループボタン配置変更**
  - BTN ROW上段: `↻LOOP ▶PAT ▶SONG ■STOP` の順に変更（LOOPを先頭に）
- **Song欄「+」ボタン命名修正**
  - Back: `makePattern('')` → `makePattern(nextPatternName())` に変更
  - RT: `makeRTPattern('')` → `makeRTPattern(nextRTPatternName())` に変更
  - ブランクパターンではなく自動命名パターンが作成される
- **パターンリネーム連動**
  - ダブルクリックリネーム時、同一 `baseName` の派生パターンも一括更新
  - Back / RT 共通処理（`targetPats` で切り替え）
...
55. v9.3: BTN ROW上段のボタン順は `↻LOOP ▶PAT ▶SONG ■STOP ↶UNDO ↷REDO ↓SAVE ↑LOAD`（LOOPが先頭）
56. v9.3: Song欄「+」ボタンはブランクパターンではなく `nextPatternName()` / `nextRTPatternName()` で自動命名したパターンを作成する
57. v9.3: パターンのダブルクリックリネームは `baseName` が一致する全派生パターン（A1, A2等）も連動更新する。Back/RT共通処理
...
v9.3           ✅ 完了（LOOPボタン配置変更 / Song「+」自動命名 / リネーム連動）
